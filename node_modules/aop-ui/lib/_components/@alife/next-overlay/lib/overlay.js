...
var _findNode = require('./utils/find-node.js');
...
var hasScroll = function hasScroll() {
    ...
    return doc.scrollHeight > doc.clientHeight && _nextUtil.dom.scrollbar().width > 0;
};
...
var Overlay = (_temp = _class = function (_Component) {
    ...
    Overlay.prototype.componentWillMount = function componentWillMount() {
        if (this.props.visible) {
            ...
            if (this.state.animation && _nextUtil.support.animation) {
                this.enter();
            }
        }
    };
    ...
    Overlay.prototype.componentWillReceiveProps = function componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.animation !== false && _nextUtil.support.animation) {
            if (willOpen) {
                this.enter();
            } else if (willClose) {
                this.leave();
        ...
        if (this.state.animation && _nextUtil.support.animation) {
            ...
            if (open) {
                setTimeout(function () {
                    ...
                    _nextUtil.dom.addClass(wrapperNode, 'opened');
                    _manager2.default.addOverlay(_this2);
                });
            } else if (close) {
                this.props.onClose();
                this.props.afterClose();
                _nextUtil.dom.removeClass(wrapperNode, 'opened');
    ...
    Overlay.prototype.addAnimationEvents = function addAnimationEvents() {
        ...
        setTimeout(function () {
            ...
            if (node) {
                var id = (0, _nextUtil.guid)();

                _this3._animation = _nextUtil.events.on(node, _nextUtil.support.animation.end, _this3.handleAnimateEnd.bind(_this3, id));
    ...
    Overlay.prototype.beforeOpen = function beforeOpen() {
        if (this.props.disableScroll) {
            ...
            this.bodyOverflowY = body.style.overflowY;
            if (hasScroll()) {
                this.bodyPaddingRight = body.style.paddingRight;
                style.paddingRight = _nextUtil.dom.getStyle(body, 'paddingRight') + _nextUtil.dom.scrollbar().width + 'px';
            }

            _nextUtil.dom.setStyle(body, style);
        }
    };
    ...
    Overlay.prototype.beforeClose = function beforeClose() {
        if (this.props.disableScroll) {
            ...
            if (hasScroll()) {
                style.paddingRight = this.bodyPaddingRight;
            }

            _nextUtil.dom.setStyle(document.body, style);

            this.bodyOverflowY = undefined;
            this.bodyPaddingRight = undefined;
        }
    };
    ...
    Overlay.prototype.addDocumentEvents = function addDocumentEvents() {
        ...
        if (this.props.canCloseByEsc) {
            this._keydownEvents = _nextUtil.events.on(document, 'keydown', this.handleDocumentKeyDown);
        }
        if (this.props.canCloseByOutSideClick) {
            var safeNode = this.props.safeNode;

            this._safeNodes = Array.isArray(safeNode) ? [].concat(safeNode) : [safeNode];
            this._safeNodes.unshift(function () {
                return _this6.getWrapperNode();
            });

            this._clickEvents = _nextUtil.events.on(document, 'click', this.handleDocumentClick);
        }
    };
    ...
    Overlay.prototype.handleDocumentKeyDown = function handleDocumentKeyDown(e) {
        if (this.state.visible && e.keyCode === _nextUtil.KEYCODE.ESC && _manager2.default.isCurrentOverlay(this)) {
    ...
    Overlay.prototype.handleDocumentClick = function handleDocumentClick(e) {
        if (this.state.visible) {
            for (var i = 0; i < this._safeNodes.length; i++) {
                var node = (0, _findNode2.default)(this._safeNodes[i], this.props);
    ...
    Overlay.prototype.render = function render() {
        ...
        if (children) {
            ...
            children = _react2.default.createElement(
                'div',
                { className: wrapperClazz, style: wrapperStyle },
                hasMask ? _react2.default.createElement('div', { className: prefix + 'overlay-backdrop', onClick: this.handleMaskClick }) : null,
                children
            );
        }
...
exports.default = _nextConfigProvider2.default.config(Overlay, {
