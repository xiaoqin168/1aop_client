...
var Position = (_temp = _class = function () {
    ...
    Position.prototype.setPosition = function setPosition() {
        ...
        if (_nextUtil.dom.getStyle(pinElement, 'position') !== 'fixed') {
            _nextUtil.dom.setStyle(pinElement, 'position', 'absolute');
            isPinFixed = false;
        } else {
            isPinFixed = true;
        }
        if (baseElement === VIEWPORT || _nextUtil.dom.getStyle(baseElement, 'position') !== 'fixed') {
            isBaseFixed = false;
        } else {
            isBaseFixed = true;
        }
        ...
        for (var i = 0; i < expectedAlign.length; i++) {
            ...
            _nextUtil.dom.setStyle(pinElement, {
                left: left + 'px',
                top: top + 'px'
            });
    ...
    Position.prototype._getParentOffset = function _getParentOffset(element) {
        ...
        var offset = void 0;
        if (parent === document.body && _nextUtil.dom.getStyle(parent, 'position') === 'static') {
            offset = {
                top: 0,
                left: 0
            };
        ...
        offset.top += parseFloat(_nextUtil.dom.getStyle(parent, 'border-top-width'), 10);
        offset.left += parseFloat(_nextUtil.dom.getStyle(parent, 'border-left-width'), 10);
    ...
    Position.prototype._makeElementInViewport = function _makeElementInViewport(pinElement, number, type, isPinFixed) {
        ...
        if (result < 0) {
            if (isPinFixed) {
                result = 0;
            } else if (offsetParent === document.body && _nextUtil.dom.getStyle(offsetParent, 'position') === 'static') {
    ...
    Position.prototype._getLeftTop = function _getLeftTop(element) {
        return {
            left: parseFloat(_nextUtil.dom.getStyle(element, 'left')) || 0,
            top: parseFloat(_nextUtil.dom.getStyle(element, 'top')) || 0
        };
    };
