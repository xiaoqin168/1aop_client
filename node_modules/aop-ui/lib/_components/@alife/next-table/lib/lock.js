...
var _row = require('./lock/row.js');

var _row2 = _interopRequireDefault(_row);

var _body = require('./lock/body.js');

var _body2 = _interopRequireDefault(_body);

var _header = require('./lock/header.js');

var _header2 = _interopRequireDefault(_header);

var _wrapper = require('./fixed/wrapper.js');
...
function lock(BaseComponent) {
    ...
    var LockTable = (_temp = _class = function (_React$Component) {
        ...
        function LockTable(props, context) {
            ...
            _this.onRowMouseEnter = function (record, index) {
                if (_this.isLock()) {
                    ...
                    [row, leftRow, rightRow].forEach(function (row) {
                        row && _nextUtil.dom.addClass(row, 'hovered');
                    });
                }
            };
            ...
            _this.onRowMouseLeave = function (record, index) {
                if (_this.isLock()) {
                    ...
                    var rightRow = _this.getRowNode(index, 'right');
                    [row, leftRow, rightRow].forEach(function (row) {
                        row && _nextUtil.dom.removeClass(row, 'hovered');
                    });
                }
            };
            ...
            _this.onLockBodyScroll = function () {
                if (_this.isLock()) {
                    ...
                    if (x === 0) {
                        lockLeftTable && _nextUtil.dom.removeClass(lockLeftTable, shadowClassName);
                        lockRightTable && _nextUtil.dom.addClass(lockRightTable, shadowClassName);
                    } else if (x === _this.bodyNode.scrollWidth - _this.bodyNode.clientWidth) {
                        lockLeftTable && _nextUtil.dom.addClass(lockLeftTable, shadowClassName);
                        lockRightTable && _nextUtil.dom.removeClass(lockRightTable, shadowClassName);
                    } else {
                        lockLeftTable && _nextUtil.dom.addClass(lockLeftTable, shadowClassName);
                        lockRightTable && _nextUtil.dom.addClass(lockRightTable, shadowClassName);
                    }
        ...
        LockTable.prototype.shouldComponentUpdate = function shouldComponentUpdate(nextProps, nextState, nextContext) {
            if (nextProps.pure) {
                ...
                return !(isEqual && _nextUtil.obj.shallowEqual(nextContext, this.context));
            }
        ...
        LockTable.prototype.adjustIfTableNotNeedLock = function adjustIfTableNotNeedLock() {
            ...
            if (this.isLock() && this.tableInc.props.dataSource.length) {
                ...
                if (configWidths <= width) {
                    if (this.lockLeftChildren.length) {
                        this._notNeedAdjustLockLeft = true;
                        this.forceUpdate();
                    }
                    if (this.lockRightChildren.length) {
        ...
        LockTable.prototype.adjustBodySize = function adjustBodySize() {
            if (this.isLock()) {
                var body = this.bodyNode,
                    ...
                    scrollbar = _nextUtil.dom.scrollbar(),
                ...
                lockLeftBody && _nextUtil.dom.setStyle(lockLeftBody, 'max-height', lockBodyHeight);
                lockRightBody && _nextUtil.dom.setStyle(lockRightBody, 'max-height', lockBodyHeight);
                lockRightBodyWrapper && _nextUtil.dom.setStyle(lockRightBodyWrapper, 'right', width);
            }
        };
        ...
        LockTable.prototype.adjustHeaderSize = function adjustHeaderSize() {
            ...
            if (this.isLock()) {
                this.tableInc.groupChildren.forEach(function (child, index) {
                    var headerRow = _this3.getHeaderCellNode(index),
                        headerRightLockRow = _this3.getHeaderCellNode(index, 'right'),
                        headerLeftLockRow = _this3.getHeaderCellNode(index, 'left'),
                    ...
                    _nextUtil.dom.setStyle(headerRow, 'height', maxRowHeight);
                    headerRightLockRow && _nextUtil.dom.setStyle(headerRightLockRow, 'height', maxRowHeight);
                    headerLeftLockRow && _nextUtil.dom.setStyle(headerLeftLockRow, 'height', maxRowHeight);
                });
            }
        };
            ...
            if (this.isLock()) {
                this.tableInc.props.dataSource.forEach(function (item, index) {
                    ...
                    if (lockLeftRow && rowHeight !== lockLeftHeight) {
                        _nextUtil.dom.setStyle(lockLeftRow, 'height', rowHeight);
                    }
                    if (lockRightRow && rowHeight !== lockRightHeight) {
                        _nextUtil.dom.setStyle(lockRightRow, 'height', rowHeight);
                    }
                });
            }
        };
        ...
        LockTable.prototype.getHeaderCellNode = function getHeaderCellNode(index, type) {
            type = type ? type.charAt(0).toUpperCase() + type.substr(1) : '';
            var table = this['table' + type + 'Inc'];
            return (0, _reactDom.findDOMNode)(table.getHeaderCellRef(index, 0));
        };
        ...
        LockTable.prototype.render = function render() {
            ...
            if (this.isLock()) {
                ...
                var content = [_react2.default.createElement(BaseComponent, (0, _extends3.default)({}, others, { key: 'lock-left', columns: lockLeftChildren, className: prefix + 'table-lock-left', prefix: prefix, lockType: 'left', components: components, ref: 'lockLeft', loading: false })), _react2.default.createElement(BaseComponent, (0, _extends3.default)({}, others, { key: 'lock-right', columns: lockRightChildren, className: prefix + 'table-lock-right', prefix: prefix, lockType: 'right', components: components, ref: 'lockRight', loading: false }))];
                return _react2.default.createElement(BaseComponent, (0, _extends3.default)({}, others, { columns: normalizedChildren, prefix: prefix, wrapperContent: content, components: components, className: className }));
            }
    ...
    (0, _util.statics)(LockTable, BaseComponent);
    return LockTable;
}
